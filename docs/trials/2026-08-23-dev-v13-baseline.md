...
the last hole in a run whose selling point is authentication; ticketed.
...
the gold *pages* were selected by browser-era run roots (escalation
stratification for pilot/batch2; `exp-second-opinion-d` for batches 3–5;
dev-v12 for batch7), so eligibility read the system under test, per the
principles §8 checklist. The *token proposals* within those pages are
...
| association coverage (mean / p50) | 0.609 / 0.684 | 0.743 / 0.905 | 0.742 / 0.901 | **unchanged** — the apparent improvement is entirely recovery in v12's denominator |
