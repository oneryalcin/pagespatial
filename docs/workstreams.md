...
| Service control plane (accounts, keys, dashboard) | #76 | **Open-alpha parse-only v1 live; compact transport Stage A implemented and live-qualified 2026-09-02.** New executions publish a canonical evidence object plus a deterministic compact companion; Node owns both serializations, Python uploads opaque bytes, and PostgreSQL accepts the validated pair atomically. Pre-migration attempts remain evidence-only and harvestable. The isolated Modal/R2 gate passed exact compact determinism, zero-tolerance output parity, digest rejection, recovery, and all five ACL probes. The public grant still returns evidence, so current clients do not change. | `feat/compact-result-transport` | service/api, deploy/control-plane, deploy/modal, compact library projection | take and verify a fresh off-site PostgreSQL dump, deploy Stage A, then wait one full two-day retention window before a separate compact-default/viewer PR |
