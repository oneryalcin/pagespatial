...
| Service control plane (accounts, keys, dashboard) | #76 | **Open-alpha parse-only v1 live; compact transport Stage A deployed 2026-09-02 at 16:07Z.** New executions publish a canonical evidence object plus a deterministic compact companion; Node owns both serializations, Python uploads opaque bytes, and PostgreSQL accepts the validated pair atomically. Pre-migration attempts remain evidence-only and harvestable. The isolated Modal/R2 gate passed exact compact determinism, zero-tolerance output parity, digest rejection, recovery, and all five ACL probes. A fresh off-site PostgreSQL dump passed byte and restore verification before migration 007; a production smoke job accepted the pair and the public grant still returned evidence, so current clients did not change. | `main` (`853ae16`) | service/api, deploy/control-plane, deploy/modal, compact library projection | no earlier than 2026-09-04 16:07Z, verify every retained post-activation success has a valid compact companion; only then open a separate compact-default/viewer PR |
