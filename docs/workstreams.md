...
| Service control plane (accounts, keys, dashboard) | #76 | **Open-alpha parse-only v1 live; compact transport Stage A implemented 2026-09-02.** New executions publish a canonical evidence object plus a deterministic compact companion; Node owns both serializations, Python uploads opaque bytes, and PostgreSQL accepts the validated pair atomically. Pre-migration attempts remain evidence-only and harvestable. The public grant still returns evidence, so current clients do not change. | `feat/compact-result-transport` | service/api, deploy/control-plane, deploy/modal, compact library projection | pass live Modal/R2 gate, back up PostgreSQL, deploy Stage A, then wait one full two-day retention window before a separate compact-default/viewer PR |
